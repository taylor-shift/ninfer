...
// coalesces. Each batch row is an independent block-diffusion sequence: the anchor column of a
// row (t % width == 0, width = C/batch_size) reads hidden[t-1,c] as zero and the conv never
// wraps across rows. All arithmetic accumulates in FP32; the single BF16 round happens at the
// store.
__global__ void grouped_dynamic_causal_conv_kernel(const __nv_bfloat16* hidden,
    ...
    for (std::int32_t t = 0; t < columns; ++t) {
        ...
        const float dyn0          = __bfloat162float(dynamic[col * kDynamicRows + group]);
        const float dyn1          = __bfloat162float(
                            dynamic[col * kDynamicRows + (kGroups + group)]);
        out[col * kChannels + c]  =
