...
std::size_t attn_input_proj_workspace_capacity_bytes(QType parent_qtype, std::int32_t parent_rows,
    ...
    case QType::W8G32_F16S:
        ...
        if (parent_rows == 6144 && input_rows == 5120) {
            // DFlash 2 (27B) QKV: the split-output MMA route needs no transient storage.
            return 0;
        }
...
void attn_input_proj(const Tensor& x, const Weight& query_key_value_weight, Tensor& q, Tensor& k,
    ...
    const std::int32_t cols        = x.ne[1];
    if (cols <= 0) { throw std::invalid_argument("attn_input_proj: T must be positive"); }
    // Two registered parent geometries share the row order [query 4096, key 1024,
    // value 1024]: the 35B companion (hidden 2048) and the 27B DFlash 2 drafter
    // (hidden 5120).
    const std::int32_t hidden = query_key_value_weight.padded_shape[1];
    if (hidden != 2048 && hidden != 5120) {
        throw std::invalid_argument("attn_input_proj: invalid query/key/value weight");
    }
    ...
    if (hidden == 5120) {
        detail::w8_dflash_qkv_dispatch(x, query_key_value_weight, q, k, v, stream);
    } else {
